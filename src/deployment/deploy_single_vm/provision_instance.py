...
def provision_instance(connection, instance, ssh_user):
    ...
    mount_disk = remote.Command(
        ...
        create="""
            ...
            if ! grep -q '/mnt/disk-1' /etc/fstab; then
                echo '/dev/disk/by-id/google-persistent-disk-1 /mnt/disk-1 ext4 discard,defaults 0 2' | sudo tee -a /etc/fstab  # noqa: F501
            fi
            # Mount the disk
            sudo mount -a
        """,
    ...
    install_docker = remote.Command(
        ...
        create="""
            ...
            # Add Docker GPG key
            curl -fsSL https://download.docker.com/linux/$DISTRO/gpg | sudo gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg  # noqa: F501
